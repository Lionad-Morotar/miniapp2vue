...
// #(2018年11月26日 10点42分)
// 紧接着 HTMLParser 我们来看 vue-template-compiler 的下一个部分

/** 换行判断正则
 * @see https://www.cnblogs.com/xiaotiannet/p/3510586.html?date=15点08分
 */
var splitRE = /\r?\n/g;
var replaceRE = /./g;
...
function parseComponent(
    ...
    // #(SFC结构)
    // 将VueJS单文件(*.vue)拆分为 template/script/style/customBlocks
    // --- Lionad: 题外话, 用源代码编辑器代码注释太累人了,
    // --- 可能理想的方法是使用自动化工具解析这些注释, 然后自动构建需要的部分
    // --- 比如, 我插入了一个标记#(标记), 则自动在文件中插入一个页脚
    // --- 或是我更新了一篇文章, 打包时自动将其加入在系列说明MD文件中某个MDList或[toc]下
    // --- [flag] 一定有开源工具的, 写完VueJS的注释就去看看, 小小期待一下
    var sfc = {
        template: null,
        script: null,
        styles: [],
    ...
    function start(
        ...
        if (depth === 0) {
            currentBlock = {
                ...
                // start 记录的时内容的start而非节点本身的start
                start: end,
                ...
                attrs: attrs.reduce(function (cumulated, ref) {
                    ...
                    // 没有值的属性默认为`true`, 如: `draggble(="true")`
            ...
            // SFC结构只允许`customBlock`和`styleBlock`有多个
            if (isSpecialTag(tag)) {
                checkAttrs(currentBlock, attrs);
                // `push`和`=`的不同, 意味着页面上可能有多个`style`节点, 而其它节点最多只可能有一个存在
                if (tag === 'style') {
        ...
        // 没进入一个新的节点(且没有退出)则增加栈深度
        if (!unary) {
            depth++;
        }
    }

    // 如果是特殊的节点, 则会检测其中特殊的属性, 将其复制到block中
    function checkAttrs(block, attrs) {
        for (var i = 0; i < attrs.length; i++) {
            // ...?这里为什么使用的是`if-else`而不是`switch`
            var attr = attrs[i];
            if (attr.name === 'lang') {
    ...
    function end(tag, start, end) {
        ...
        // 在#(start)函数中, depth++, 所以`end`函数中, depth 为1时才代表顶层节点
        if (depth === 1 && currentBlock) {
            ...
            // 因为顶层节点被拆分为了SFC结构, 所以, 非template节点需要进行填充处理,
            // 主要是处理'\n'逻辑, 以保证信息(如报错信息)提示的代码行数处于编辑器所见的位置
            // --- 百度翻译: `pad content: 焊盘含量` !? hhh
    ...
    // 默认填充规则会寻找从content开始到content结束间的所有`/\r?\n/`
